const CACHE_NAME='deutschpath-pwa-v4.0.7';
const APP_SHELL=[
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('deutschpath-pwa-')&&key!==CACHE_NAME).map(key=>caches.delete(key)))).then(()=>self.clients.claim()));
});
